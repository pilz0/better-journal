{
  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      {
        packages = {
          default = self.packages.${system}.apk-release;
          apk-release = gradle2nix.builders.${system}.buildGradlePackage {
          ...
          apk-debug = gradle2nix.builders.${system}.buildGradlePackage {
            pname = "freaklog apk debug";
            version = "11.13";
            lockFile = ./gradle.lock;
            src = ./.;
            gradleBuildFlags = [ "assembleDebug" ];
            ANDROID_HOME = "${androidSdk.androidsdk}/libexec/android-sdk";
            preBuild = ''
              rm -f local.properties
              export ANDROID_USER_HOME=$(mktemp -d)
            ...
            installPhase = ''
              mkdir -p $out/bin
              cp app/build/outputs/apk/debug/*.apk $out/bin/
            '';
          };
